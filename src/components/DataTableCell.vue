<script>
    export default{
        ...
        render(createElement){
            if(this.comp){
                let props = {name: this.name,data: this.value,meta: this.meta};
                props[this.event] = this.handler;
                return createElement(this.comp,{props,attrs: {classes: this.classes}});
            }
            if(this.transform){
                return createElement('span',{
                    domProps:{
                        innerHTML: this.transform({data: this.value,name: this.name,row: this.row,column: this.column,meta: this.meta})
                    }
                });
            }
